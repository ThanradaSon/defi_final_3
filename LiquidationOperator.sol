...
contract LiquidationOperator is IUniswapV2Callee {
    ...
    IERC20 constant USDC = IERC20(0x7EA2be2df7BA6E54B1A9C70676f668455E329d29);
    ...
    IUniswapV2Pair immutable uniswapV2Pair_USDC_USDT; // Pool2
    ...
    constructor() {
        ...
        uniswapV2Pair_USDC_USDT = IUniswapV2Pair(uniswapV2Factory.getPair(address(USDC), address(USDT))); // Pool2
        debt_USDC = 8128000000000;
    ...
    function operate() external {
        ...
        uniswapV2Pair_WETH_USDC.swap(0, debt_USDC, address(this), "$");
    ...
    function uniswapV2Call(
        ...
        (uint256 reserve_WETH_Pool1, uint256 reserve_USDC_Pool1, ) = uniswapV2Pair_WETH_USDC.getReserves(); // Pool1
        (uint256 reserve_USDC_Pool2, uint256 reserve_USDT_Pool2, ) = uniswapV2Pair_USDC_USDT.getReserves(); // Pool2

        // 2.1 liquidate the target user
        
        uint debtToCover = amount1;
        USDC.approve(address(lendingPool), debtToCover);
        lendingPool.liquidationCall(address(WETH), address(USDT), liquidationTarget, debtToCover, false);
        uint collateral_WETH = WETH.balanceOf(address(this));

        // 2.2 swap WBTC for other things or repay directly
        
        WETH.transfer(address(uniswapV2Pair_WETH_USDC), collateral_WETH);
        uint amountOut_WETH = getAmountOut(collateral_WETH, reserve_USDC_Pool2, reserve_USDT_Pool2);
